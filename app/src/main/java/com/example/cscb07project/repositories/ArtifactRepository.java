...
public class ArtifactRepository implements ArtifactInterface {
    ...
    public Task<Void> addArtifact(Artifact artifact) {
        ...
        String artifactId = dbRef.push().getKey(); // fetch autogenerated ID after every post
        if (artifactId == null) throw new IllegalStateException();
        artifact.setArtifactId(artifactId);
        return dbRef.child(artifactId).setValue(artifact).continueWithTask(snapshot -> {
            if (!snapshot.isSuccessful()) throw Objects.requireNonNull(snapshot.getException());
            return expandedViewRepository.addExpandedView(new ExpandedView(artifact.getLotNumber()));
        });
    }
